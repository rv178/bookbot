...
module.exports = {
  ...
  run: async(client, interaction, args) => {
    ...
    if(!data) {
        ...
        .setAuthor({ name: `Please pick a genre from /set-genre to start!`, iconURL: interaction.user.avatarURL({dynamic:true}) })
        .setColor("BLUE");
    ...
    if (data) {
        if (data.Read.includes(book)) {
            ...
            .setAuthor({ name: `You have read ${book}`, iconURL: interaction.user.avatarURL({dynamic:true}) })
            .setFooter({text: `Author: ` + await bookAuthor(bookInfo)})
