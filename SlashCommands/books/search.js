	...
	run: async (client, interaction, args) => {
		...
		const bookInfo = await axios
			...
			.catch(function (error) {
				return interaction.followUp({ content: `No books found. Error: ${error}`});
			});
		...
		const thumbnail = bookInfo.data.items[0].volumeInfo.imageLinks.thumbnail;
